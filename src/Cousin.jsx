        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "12px", color: "#64748b" }}>
              You · <span style={{ color: "#94a3b8" }}>{myCity}</span>
            </span>
            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "12px", fontWeight: "600", color: "#0f172a" }}>
              {getTimeAt(myTz, simulatedNow)}
            </span>
          </div>
          {/* Each selected contact */}
          {selectedContacts.map(c => {
            const status = getCallStatus(c.tz, simulatedNow, c.busyDuringWork);
            return (
              <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "12px", color: "#64748b" }}>
                  {c.name.split(" ")[0]} · <span style={{ color: "#94a3b8" }}>{c.city}</span>
                </span>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "11px", color: status.green ? "#10b981" : "#94a3b8" }}>
                    {status.label}
                  </span>
                  <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "12px", fontWeight: "600", color: "#0f172a" }}>
                    {getTimeAt(c.tz, simulatedNow)}
                  </span>
                </div>
              </div>
            );
